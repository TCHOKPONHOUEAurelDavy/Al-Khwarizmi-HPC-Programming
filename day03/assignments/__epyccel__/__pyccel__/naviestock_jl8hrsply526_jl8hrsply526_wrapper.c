...
int64_t bind_c_build_up_b_numba(int64_t n0_b, int64_t n1_b, double *b, double rho, double dt, int64_t n0_u, int64_t n1_u, double *u, int64_t n0_v, int64_t n1_v, double *v, double dx, double dy);
int64_t bind_c_pressure_poisson_numba(int64_t n0_p, int64_t n1_p, double *p, double dx, double dy, int64_t n0_b, int64_t n1_b, double *b, int64_t nt);
int64_t bind_c_cavity_flow_numba(int64_t nt, int64_t n0_u, int64_t n1_u, double *u, int64_t n0_v, int64_t n1_v, double *v, double dt, int64_t nx, int64_t ny, double dx, double dy, int64_t n0_p, int64_t n1_p, double *p, double rho, double nu);
...
PyObject *build_up_b_numba_wrapper(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    Out_0001 = bind_c_build_up_b_numba(nd_ndim(&b, 0), nd_ndim(&b, 1), nd_data(&b), rho, dt, nd_ndim(&u, 0), nd_ndim(&u, 1), nd_data(&u), nd_ndim(&v, 0), nd_ndim(&v, 1), nd_data(&v), dx, dy);
...
PyObject *pressure_poisson_numba_wrapper(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    Out_0002 = bind_c_pressure_poisson_numba(nd_ndim(&p, 0), nd_ndim(&p, 1), nd_data(&p), dx, dy, nd_ndim(&b, 0), nd_ndim(&b, 1), nd_data(&b), nt);
...
PyObject *cavity_flow_numba_wrapper(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OO!O!OOOOOO!OO", kwlist, &nt_tmp, &PyArray_Type, &u_tmp, &PyArray_Type, &v_tmp, &dt_tmp, &nx_tmp, &ny_tmp, &dx_tmp, &dy_tmp, &PyArray_Type, &p_tmp, &rho_tmp, &nu_tmp))
    {
        return NULL;
    }
    if (PyIs_NativeInt(nt_tmp))
    ...
    Out_0003 = bind_c_cavity_flow_numba(nt, nd_ndim(&u, 0), nd_ndim(&u, 1), nd_data(&u), nd_ndim(&v, 0), nd_ndim(&v, 1), nd_data(&v), dt, nx, ny, dx, dy, nd_ndim(&p, 0), nd_ndim(&p, 1), nd_data(&p), rho, nu);
...
static PyMethodDef naviestock_jl8hrsply526_jl8hrsply526_methods[] = {
    {
        "build_up_b_numba",
...
static PyModuleDef_Slot naviestock_jl8hrsply526_jl8hrsply526_slots[] = {
...
static struct PyModuleDef naviestock_jl8hrsply526_jl8hrsply526_module = {
    PyModuleDef_HEAD_INIT,
    /* name of module */
    "naviestock_jl8hrsply526_jl8hrsply526",
    /* module documentation, may be NULL */
    NULL,
    /* size of per-interpreter state of the module, or -1 if the module keeps state in global variables. */
    0,
    naviestock_jl8hrsply526_jl8hrsply526_methods,
    naviestock_jl8hrsply526_jl8hrsply526_slots
};

/*........................................*/

PyMODINIT_FUNC PyInit_naviestock_jl8hrsply526_jl8hrsply526(void)
{
    import_array();
    return PyModuleDef_Init(&naviestock_jl8hrsply526_jl8hrsply526_module);
}
