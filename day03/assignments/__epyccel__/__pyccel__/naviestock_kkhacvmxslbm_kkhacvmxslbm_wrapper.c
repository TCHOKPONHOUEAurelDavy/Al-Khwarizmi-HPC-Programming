...
int64_t bind_c_pressure_poisson_numba(int64_t n0_p, int64_t n1_p, double *p, double dx, double dy, int64_t n0_b, int64_t n1_b, double *b, double nit, double nt);
int64_t bind_c_cavity_flow_numba(int64_t n0_u, int64_t n1_u, double *u, int64_t n0_v, int64_t n1_v, double *v, double dt, int64_t nx, int64_t ny, double dx, double dy, int64_t n0_p, int64_t n1_p, double *p, double rho, double nu, double nit, double nt);
...
PyObject *pressure_poisson_numba_wrapper(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (PyIs_NativeFloat(nit_tmp))
    {
        nit = PyDouble_to_Double(nit_tmp);
    }
    else
    {
        PyErr_SetString(PyExc_TypeError, "\"Argument must be native float\"");
    ...
    if (PyIs_NativeFloat(nt_tmp))
    {
        nt = PyDouble_to_Double(nt_tmp);
    }
    else
    {
        PyErr_SetString(PyExc_TypeError, "\"Argument must be native float\"");
    ...
    Out_0002 = bind_c_pressure_poisson_numba(nd_ndim(&p, 0), nd_ndim(&p, 1), nd_data(&p), dx, dy, nd_ndim(&b, 0), nd_ndim(&b, 1), nd_data(&b), nit, nt);
...
PyObject *cavity_flow_numba_wrapper(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    Out_0003 = bind_c_cavity_flow_numba(nd_ndim(&u, 0), nd_ndim(&u, 1), nd_data(&u), nd_ndim(&v, 0), nd_ndim(&v, 1), nd_data(&v), dt, nx, ny, dx, dy, nd_ndim(&p, 0), nd_ndim(&p, 1), nd_data(&p), rho, nu, nit, nt);
...
static PyMethodDef naviestock_kkhacvmxslbm_kkhacvmxslbm_methods[] = {
    {
        "build_up_b_numba",
...
static PyModuleDef_Slot naviestock_kkhacvmxslbm_kkhacvmxslbm_slots[] = {
...
static struct PyModuleDef naviestock_kkhacvmxslbm_kkhacvmxslbm_module = {
    PyModuleDef_HEAD_INIT,
    /* name of module */
    "naviestock_kkhacvmxslbm_kkhacvmxslbm",
    /* module documentation, may be NULL */
    NULL,
    /* size of per-interpreter state of the module, or -1 if the module keeps state in global variables. */
    0,
    naviestock_kkhacvmxslbm_kkhacvmxslbm_methods,
    naviestock_kkhacvmxslbm_kkhacvmxslbm_slots
};

/*........................................*/

PyMODINIT_FUNC PyInit_naviestock_kkhacvmxslbm_kkhacvmxslbm(void)
{
    import_array();
    return PyModuleDef_Init(&naviestock_kkhacvmxslbm_kkhacvmxslbm_module);
}
